...
class TransactionViewTests(TestCase):
    ...
    def test_transfer_funds_insufficient_balance(self):
        """Testa a transferência de fundos sem saldo suficiente."""
        url = '/api/transfer/'
        data = {
            "receiver": self.receiver.username,
            "amount": 100
    ...
    def test_transfer_funds_nonexistent_receiver(self):
        """Testa a transferência de fundos para um receptor inexistente."""
        url = '/api/transfer/'
        data = {
            "receiver": "usuario_inexistente",
            "amount": 100
        }
        response = self.client.post(url, data, format='json')
